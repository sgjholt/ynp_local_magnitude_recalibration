...
def remove_mixed_station_meta_rows(df: pd.DataFrame,
    ...
    """
    A function to remove rows that have mixed station label and location
    information. This is an inplace operation.

    Parameters
    ----------
    df : pandas.DataFrame
        A pandas DataFrame of amplitudes.
    ...
    Notes
    -----
    For some unexplainable reason certain rows have mixed station and
    station location metadata. Since there is no way to tell whether the
    station or station locations are correct, we have to remove those
    rows. We simply just determine if latitude is the same as that obtained
    from IRIS's metadata aggregator. If its different to more than 0.001
    degrees, the row is thrown out.
    """
