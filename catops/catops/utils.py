...
def clear_issues(df:pd.DataFrame, funcs) -> None:
    """
...
def dataframe_difference(df1: pd.DataFrame, df2: pd.DataFrame, which=None) -> pd.DataFrame:
...
def quick_inspect_magnitude_distance(df: pd.DataFrame, save:Union[str,bool]=False) -> None:
    inspec = df.copy(deep=True)
    inspec = inspec[inspec.CatMag != -9.99]
    M, Dist, Dep, A = inspec.CatMag, inspec.Rhyp, inspec.drop_duplicates("Evid").EqDep, np.log10(inspec.halfAmpH)
...
def quick_inspect_spatial_distribution(df: pd.DataFrame, save:Union[str,bool]=False) -> None:
    inspec = df.copy(deep=True)
    Lon, Lat, Dep = inspec.EqLon.values, inspec.EqLat.values, inspec.EqDep.values
