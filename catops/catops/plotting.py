...
def quick_inspect_amplitudes(N:np.array, E:np.array, C:np.array, s=1, logx=False, logy=False, loglog=False, cmap='viridis', **kwargs) -> None:
    ...
    xlabel=f"log10 p-p E-W [mm]"
    ylabel=f"log10 p-p N-S [mm]"
...
def magnitude_distance_plot(M: np.array, Dist: np.array, Dep: np.array, A: np.array):
    """

    """
    hkwargs = dict(bottom=0.0, color='.8', edgecolor='k', rwidth=0.8, weights=np.zeros_like(Dist) + 1. / len(Dist))
    hdepkwargs = dict(bottom=0.0, color='.8', edgecolor='k', rwidth=0.8, weights=np.zeros_like(Dep) + 1. / len(Dep))
    ## figure stuff
    fac = 2.6
...
#     ax2.xaxis.set_ticks([1, 10, 100, 1000])
    ...
    a1 = ax1.hist(np.log10(Dist), **hkwargs)
    a3 = ax3.hist(M, orientation='horizontal', **hkwargs)
    a4 = ax4.hist(Dep, **hdepkwargs)
    sout = ax2.scatter(Dist, M, c=A, lw=1, cmap='viridis', s=10)

    cbaxes = inset_axes(ax2, width="2.5%", height="55%", loc=2)
    cbar = fig.colorbar(sout, cax=cbaxes)
    cbar.set_label("log10(A[mm])", rotation=90)

def spatial_distribution_plot(Lon: np.array, Lat: np.array, Dep: np.array) -> None:

    hkwargs = dict(bottom=0.0, color='.8', edgecolor='k', rwidth=0.8, weights=np.zeros_like(Lon) + 1. / len(Lon))
    ...
    a1 = ax1.hist(Lon, **hkwargs)
    a3 = ax3.hist(Lat, orientation='horizontal', **hkwargs)
    a4 = ax4.hist(Dep, **hkwargs)
